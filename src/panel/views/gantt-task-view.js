...
function getInterval(task, mode) {
    if (mode === 'starts-done') {
        const s = task._starts, e = task._done || task._due;
        return s && e ? { start: new Date(s), end: new Date(e) } : null;
    }
    const s = task._scheduled, e = task._due;
    return s && e ? { start: new Date(s), end: new Date(e) } : null;
}
...
function buildTree(tasks, dv) {
    ...
    map.forEach((ts, path) => {
        ...
        let parent = null, curPath = '';
        parts.forEach((part, i) => {
            const last = i === parts.length - 1;
            curPath += (curPath ? '/' : '') + part;
            const full = prefix + curPath;
            if (last) {
                const node = { type: 'file', name, path, tasks: ts, fullPath: full, parent, children: [] };
                if (parent) parent.children.push(node); else roots.push(node);
            } else {
                let folder = parent?.children.find(n => n.type === 'folder' && n.name === part);
                if (!folder) {
                    folder = { type: 'folder', name: part, children: [], parent, fullPath: full };
    ...
    nodes.forEach(n => {
        if (n.type === 'file' && n.tasks) {
            n.tasks.sort((a, b) => {
                ...
                if (sort.type === 'status') {
                    const m = { todo:0, planned:1, 'in-progress':2, cancelled:3, completed:4 };
                    return (m[a._status] - m[b._status]) * o;
                }
                if (sort.type === 'priority') {
                    const pa = a._priorityIcon ? { '🔺':0, '⏫':1, '🔼':2, '🔽':3, '⏬':4 }[a._priorityIcon] : 5;
                    const pb = b._priorityIcon ? { '🔺':0, '⏫':1, '🔼':2, '🔽':3, '⏬':4 }[b._priorityIcon] : 5;
                    return (pa - pb) * o;
                }
    ...
    nodes.forEach(n => {
        ...
        if (n.type === 'file') {
            if (!collapsed.has(n.fullPath) && n.tasks) {
                n.tasks.forEach(task => out.push({ type: 'task', task, file: n, level: level + 1 }));
            }
        } else if (n.type === 'folder') {
            if (!collapsed.has(n.fullPath) && n.children) flatten(n.children, level + 1, out, collapsed);
        }
    });
}
...
function prepareTasks(flat, intervalMode) {
    ...
    flat.forEach((node, idx) => {
        ...
        if (!iv) return;
        rows.push({ task: node.task, start: iv.start, end: iv.end, status: node.task._status, dur: iv.end - iv.start, flatIdx: idx });
    });
    return rows;
}
...
function ensureTheme() {
    ...
    const fontFam = style.getPropertyValue('--font-text') || 'Inter, sans-serif';
    gs.theme = {
        text: style.getPropertyValue('--text-normal').trim() || '#333',
        grid: style.getPropertyValue('--background-modifier-border').trim() || '#ccc',
        accent: style.getPropertyValue('--text-accent').trim() || '#4dabf7',
        bg: style.getPropertyValue('--background-primary').trim() || '#fff',
        secondary: style.getPropertyValue('--background-secondary').trim() || '#f0f0f0',
        font: `13px ${fontFam}`,
        boldFont: `bold 13px ${fontFam}`,
        fontFam
    };
    if (!gs.barPath) {
        const r = 4, w = 100, h = 20;
        const p = new Path2D();
...
function draw() {
    ...
    ctx.beginPath(); ctx.moveTo(gs.leftW, 0); ctx.lineTo(gs.leftW, h); ctx.stroke();
    ...
    const endRow = Math.min(nodes.length, Math.ceil((gs.scrollTop + h) / rowH) + 2);
    ...
    for (let i = startRow; i < endRow; i++) {
        ...
        if (node.type === 'folder' || node.type === 'file') {
            ...
            ctx.fillText((node.type === 'folder' ? '📁 ' : '📄 ') + node.name + (node.tasks ? ` (${node.tasks.length})` : ''), x, y);
        } else if (node.type === 'task') {
            const task = node.task;
            x += 14;
            if (isToday(task)) { ctx.fillStyle = '#f59f00'; ctx.fillText('●', x, y - 2); }
            x += 12;
            ctx.fillStyle = theme.text;
            ctx.font = theme.font;
    ...
    if (now >= gs.minTime && now <= gs.maxTime) {
        ...
        ctx.strokeStyle = '#f59f00'; ctx.lineWidth = 1.5; ctx.setLineDash([3,3]); ctx.stroke(); ctx.setLineDash([]);
    }

    // 任务条
    rows.forEach(item => {
        if (item.flatIdx < startRow || item.flatIdx >= endRow) return;
        const x1 = gs.timeToX(item.start.getTime()), x2 = gs.timeToX(item.end.getTime());
        const y = yOff + item.flatIdx * rowH + (rowH - 20) / 2;
        let bw = x2 - x1;
        if (bw < 2) bw = 2;
        ctx.fillStyle = STATUS_COLORS[item.status] || '#adb5bd';
        ctx.save(); ctx.translate(x1, y); ctx.scale(bw / 100, 1); ctx.fill(gs.barPath); ctx.restore();
        const dur = formatDur(item.dur);
        if (dur) {
            ctx.fillStyle = theme.text; ctx.font = '10px ' + theme.fontFam; ctx.textAlign = 'left';
            const tx = x2 + 4;
    ...
    ctx.strokeStyle = depColor; ctx.lineWidth = 1; ctx.fillStyle = depColor;
    rows.forEach(item => {
        ...
        deps.forEach(depId => {
            // 查找依赖任务（通过 _id 标识）
            const depTask = gs.flatNodes.find(n => n.type === 'task' && n.task._id === depId)?.task;
            if (!depTask) return;
            const depRow = rows.find(r => r.task === depTask);
            if (!depRow) return;
            // 箭头：从 depTask 的完成/截止 -> 当前 task 的开始/计划
    ...
    // 顶部日期标签
    ctx.fillStyle = theme.text; ctx.font = '11px ' + theme.fontFam; ctx.textAlign = 'center';
    for (let i = 0; i <= 10; i++) {
        const time = gs.minTime + (i / 10) * (gs.maxTime - gs.minTime);
        const x = gs.timeToX(time);
        if (x > gs.leftW && x < w) ctx.fillText(formatDate(new Date(time)), x, 14);
    }
}
...
function isToday(task) {
    ...
    const test = d => d ? (new Date(d).getTime() >= today && new Date(d).getTime() < tomorrow) : false;
    return test(task._scheduled) || test(task._due) || test(task._starts) || test(task._created);
}
        ...
        if (node.type === 'file') {
            ...
            if (file) {
                ...
                leaf.openFile(file).then(() => {
                    setTimeout(() => leaf.view?.editor?.setCursor({ line: task.line ?? 0, ch: 0 }), 20);
                });
            }
...
function render(dv, app, globalState) {
    ...
    const { dateFilterState, markFilterState, hideRepeatTasks, hideCompletedTasks, hideCancelledTasks } = globalState || {};
    if (dateFilterState && !dateFilterState.isAll && dateFilterState.start && dateFilterState.end) {
        const rs = dateFilterState.start.getTime(), re = dateFilterState.end.getTime();
        tasks = tasks.filter(t => { const tr = t._cachedTimeRange; return tr && tr.start <= re && tr.end >= rs; });
    }
    if (markFilterState?.statuses?.length < CONFIG.ALLOWED_STATUSES.length) tasks = tasks.filter(t => markFilterState.statuses.includes(t._status));
    ...
    if (hideCompletedTasks) tasks = tasks.filter(t => t._status !== 'completed');
    if (hideCancelledTasks) tasks = tasks.filter(t => t._status !== 'cancelled');
...
export async function startGanttView(dv, app, container, globalState) {
    ...
    folderBtn.textContent = gs.collapsed.size ? '📁 显示文件夹' : '📂 隐藏文件夹';
    folderBtn.onclick = () => {
        if (gs.collapsed.size) { gs.collapsed.clear(); } else { gs.flatNodes.forEach(n => { if (n.type === 'folder' || n.type === 'file') gs.collapsed.add(n.fullPath); }); }
        folderBtn.textContent = gs.collapsed.size ? '📁 显示文件夹' : '📂 隐藏文件夹';
    ...
    canvas.addEventListener('mousedown', e => {
        ...
        if (Math.abs(x - gs.leftW) < 6) {
            gs.isResizing = true; canvas.style.cursor = 'ew-resize'; e.preventDefault();
        } else {
            gs.isDragging = true; gs.lastDragX = e.clientX; canvas.style.cursor = 'grabbing';
        }
    });
    ...
    window.addEventListener('mouseup', () => {
        gs.isResizing = false; gs.isDragging = false; canvas.style.cursor = 'default';
    });
    ...
    globalState.dateFilterState = { isAll: false, start: thisWeek.start, end: thisWeek.end };
    render(dv, app, globalState);

    return {
        cleanup: () => {
            container.innerHTML = '';
            if (gs.tooltip) { gs.tooltip.remove(); gs.tooltip = null; }
            if (gs.resizeObs) { gs.resizeObs.disconnect(); gs.resizeObs = null; }
        },
