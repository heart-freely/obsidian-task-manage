...
const {
    formatDate, setStart, setEnd, getWeekRange, getISOWeekNumber, getWeekOfMonth,
    getWeekRangeByYearWeek, getMonthRangeByYearMonth, getQuarterRangeByYearQuarter,
    getYearRangeByYear, getWeekdayRange
} = DateUtils;
...
function safeState(globalState) {
    return {
        ...
        markFilterState: {
            statuses: globalState?.markFilterState?.statuses ?? [...CONFIG.ALLOWED_STATUSES],
...
function getFilteredTasks(dv, globalState) {
    ...
    if (markFilterState.statuses.length < CONFIG.ALLOWED_STATUSES.length) {
        tasks = tasks.filter(t => markFilterState.statuses.includes(t._status));
    }
    if (markFilterState.includeMarks.length) tasks = tasks.filter(t => markFilterState.includeMarks.every(m => t._marks?.[m]));
    if (markFilterState.excludeMarks.length) tasks = tasks.filter(t => !markFilterState.excludeMarks.some(m => t._marks?.[m]));
    if (state.hideRepeatTasks) tasks = tasks.filter(t => !t._repeat);
    if (state.hideCompletedTasks) tasks = tasks.filter(t => t._status !== 'completed');
    if (state.hideCancelledTasks) tasks = tasks.filter(t => t._status !== 'cancelled');

    tasks.sort((a, b) => {
        const pa = a._priorityIcon ? { '⏬':4, '🔽':3, '🔼':2, '⏫':1, '🔺':0 }[a._priorityIcon] : 5;
        const pb = b._priorityIcon ? { '⏬':4, '🔽':3, '🔼':2, '⏫':1, '🔺':0 }[b._priorityIcon] : 5;
        if (pa !== pb) return pa - pb;
        const om = { todo:0, planned:1, 'in-progress':2, cancelled:3, completed:4 };
...
function getEffectiveDateRange(state, dv) {
    if (state.dateFilterState.isAll || !state.dateFilterState.start || !state.dateFilterState.end) {
        ...
        return min ? { start: setStart(new Date(min)), end: setEnd(new Date(max)) } : null;
    }
...
function buildDateTaskMap(tasks, state) {
    const map = {};
    tasks.forEach(task => {
        const dateFields = ['_created', '_scheduled', '_starts', '_due', '_done', '_cancel'];
...
function buildGlobalTitle(state, groupCount, taskCount) {
    ...
    if (!state.dateFilterState.isAll && state.dateFilterState.start && state.dateFilterState.end) {
        ...
        parts.push(`${y}年 第${q}季度 ${m}月 第${wom}周 周${wdStr} ${formatDate(s)}~${formatDate(e)}`);
    } else {
        parts.push('全部日期');
    }
    if (state.markFilterState.statuses.length < CONFIG.ALLOWED_STATUSES.length) {
        parts.push(state.markFilterState.statuses.map(s => CONFIG.STATUS_NAMES[s]).join('，'));
    }
    if (state.markFilterState.includeMarks.length) parts.push('包含：' + state.markFilterState.includeMarks.map(m => CONFIG.MARK_NAMES[m]).join('，'));
    if (state.markFilterState.excludeMarks.length) parts.push('排除：' + state.markFilterState.excludeMarks.map(m => CONFIG.MARK_NAMES[m]).join('，'));
...
function renderDayView(container, tasks, globalState, app, dv, renderFn) {
    container.innerHTML = '';
    if (!tasks.length) {
        container.innerHTML = '<div class="empty-message">📭 无符合条件的任务</div>';
        return;
    }
    ...
    if (calState.singleDateMode && calState.focusedDate) {
        ...
        if (!range) { container.innerHTML = '<div class="empty-message">📭 无法确定日期范围</div>'; return; }
    ...
    dates.forEach(date => {
        ...
        const dayTasks = tasks.filter(t => {
            const tr = t._cachedTimeRange;
            return tr && tr.start <= setEnd(date).getTime() && tr.end >= setStart(date).getTime();
        });
        ...
        dayTasks.sort((a, b) => {
            if (sort.type === 'priority') {
                ...
                const om = { todo:0, planned:1, 'in-progress':2, cancelled:3, completed:4 };
                return (sort.order === 'asc' ? 1 : -1) * ((om[a._status]||99) - (om[b._status]||99));
            } else {
                ...
                if (!va) return 1;
                if (!vb) return -1;
                return sort.order === 'desc' ? new Date(vb) - new Date(va) : new Date(va) - new Date(vb);
            }
        });
        ...
        dayTasks.forEach(task => {
            const cardData = normalizeTaskCardData({
                ...
                id: task._id || '',
                forbid: task._forbid || '',
                fileName: (task.path || '').split('/').pop().replace(/\.md$/, ''),
                path: task.path,
                lineNumber: task.line || 0
            });
            const card = createTaskCard(cardData, app);
            if (calState.focusedTaskId && (task.line + '@' + task.path) === calState.focusedTaskId) {
...
function renderWeekView(container, tasks, globalState, app, dv, renderFn) {
    ...
    while (cur.getTime() <= end) {
        ...
        if (!weeks.some(w => w.start.getTime() === wr.start.getTime())) weeks.push(wr);
        cur.setDate(cur.getDate() + 7);
    }

    const dateTaskMap = buildDateTaskMap(tasks, state);
    const globalOrderIndex = {};
    tasks.forEach((t, i) => { globalOrderIndex[t.line + '@' + t.path] = i; });
    ...
    const flushNoTaskBuffer = () => {
        ...
        const subtitle = startWeek === endWeek ? `第${startWeek}周` : `第${startWeek}周 - 第${endWeek}周`;
    ...
    weeks.forEach(week => {
        ...
        const hasTasks = tasks.some(t => {
            const tr = t._cachedTimeRange;
            return tr && tr.start <= weekEnd.getTime() && tr.end >= weekStart.getTime();
        });
        ...
        const weekTasks = Array.from(weekTaskSet).sort((a, b) => (globalOrderIndex[a.line+'@'+a.path]||999) - (globalOrderIndex[b.line+'@'+b.path]||999));

        const weekBlock = document.createElement('div');
        weekBlock.className = 'week-block';
        weekBlock.appendChild(Object.assign(document.createElement('div'), {
            ...
            weekTasks.forEach(task => {
                ...
                if (inDay) {
                    ...
                    if (showDesc) {
                        ...
                        desc.textContent = (CONFIG.STATUS_ICONS[task._status] || '') + ' ' + (task._cleanText || task.text);
                        desc.title = task._tooltip || task._cleanText;
                        desc.onclick = (e) => { e.stopPropagation(); handleTaskClick(task, day); };
                        row.appendChild(desc);
                    } else {
                        const line = document.createElement('div');
                        line.className = `cal-task-line ${task._status}`;
                        line.title = task._tooltip || task._cleanText;
                        line.onclick = (e) => { e.stopPropagation(); handleTaskClick(task, day); };
    ...
    // 计算有任务的组数
    const groupCount = items.filter(item => item.classList.contains('week-block')).length;
    container.dataset.groupCount = groupCount;
    container.dataset.taskCount = tasks.length;
}
...
function renderMonthView(container, tasks, globalState, app, dv, renderFn) {
    ...
    months.forEach(month => {
        ...
        const hasTasks = tasks.some(t => {
            const tr = t._cachedTimeRange;
            return tr && tr.start <= monthEnd.getTime() && tr.end >= monthStart.getTime();
        });
        if (!hasTasks) {
        ...
        ['一','二','三','四','五','六','日'].forEach(d => {
            grid.appendChild(Object.assign(document.createElement('div'), { style: 'text-align:center; font-weight:bold; padding:4px 0;', textContent: d }));
        });

        const firstDay = new Date(year, mon, 1);
        const dow = firstDay.getDay() || 7;
        const startDay = new Date(firstDay); startDay.setDate(1 - (dow === 7 ? 6 : 1 - dow));
        const todayStr = formatDate(new Date());
        ...
        for (let i = 0; i < 42; i++) {
            ...
            const cell = document.createElement('div');
            cell.className = 'cal-cell' + (isOtherMonth ? ' other-month' : '') + (isToday ? ' today' : '');
            const header = document.createElement('div');
            header.className = 'cal-cell-header';
            ...
            displayedTasks.forEach(task => {
                ...
                if (showDesc) {
                    ...
                    desc.textContent = (CONFIG.STATUS_ICONS[task._status] || '') + ' ' + (task._cleanText || task.text);
                    desc.title = task._tooltip || task._cleanText;
                    desc.onclick = (e) => { e.stopPropagation(); handleTaskClick(task, d); };
                    row.appendChild(desc);
                } else {
                    const line = document.createElement('div');
                    line.className = `cal-task-line ${task._status}`;
                    line.title = task._tooltip || task._cleanText;
                    line.onclick = (e) => { e.stopPropagation(); handleTaskClick(task, d); };
                    row.appendChild(line);
                }
                cell.appendChild(row);
            });
    ...
    const groupCount = items.filter(item => item.classList.contains('week-block') === false && item.querySelector('.calendar-grid')).length;
    container.dataset.groupCount = groupCount;
    container.dataset.taskCount = tasks.length;
}
...
function renderQuarterView(container, tasks, globalState, app, dv, renderFn) {
    ...
    const end = range.end || new Date();
    while (cur <= end) {
        const y = cur.getFullYear(), q = Math.floor(cur.getMonth() / 3) + 1;
        if (!quarters.some(i => i.year === y && i.quarter === q)) quarters.push({ year: y, quarter: q });
    ...
    quarters.forEach(qu => {
        ...
        const block = document.createElement('div'); block.style.marginBottom = '32px';
        block.appendChild(Object.assign(document.createElement('div'), { style: 'font-weight:bold; color:var(--text-accent);', textContent: `${qu.year}年 第${qu.quarter}季度` }));
        const sm = (qu.quarter - 1) * 3;
        for (let mo = 0; mo < 3; mo++) {
            const monthIdx = sm + mo;
            const monDiv = document.createElement('div'); monDiv.style.marginBottom = '16px';
            monDiv.appendChild(Object.assign(document.createElement('div'), { style: 'font-weight:bold;', textContent: `${qu.year}年${monthIdx + 1}月` }));
...
function renderYearView(container, tasks, globalState, app, dv, renderFn) {
    ...
    const dateTaskMap = buildDateTaskMap(tasks, state);
    const todayStr = formatDate(new Date());
    const maxCount = Math.max(1, ...Object.values(dateTaskMap).map(arr => arr.length));

    years.forEach(year => {
        const yearBlock = document.createElement('div'); yearBlock.style.marginBottom = '32px';
        yearBlock.appendChild(Object.assign(document.createElement('div'), { style: 'font-size:1.4em; font-weight:bold;', textContent: `${year}年` }));
        const grid = document.createElement('div'); grid.className = 'year-grid';
        for (let m = 0; m < 12; m++) {
            const monthDiv = document.createElement('div'); monthDiv.className = 'year-month-card';
            monthDiv.appendChild(Object.assign(document.createElement('div'), { className: 'year-month-title', textContent: `${m + 1}月` }));
            const heatGrid = document.createElement('div'); heatGrid.className = 'year-heat-grid';
            const firstDay = new Date(year, m, 1);
            const dow = firstDay.getDay() || 7;
            const startDay = new Date(firstDay); startDay.setDate(1 - (dow === 7 ? 6 : 1 - dow));
            for (let d = 0; d < 42; d++) {
                const curDate = new Date(startDay); curDate.setDate(startDay.getDate() + d);
                const dateStr = formatDate(curDate);
                const count = (dateTaskMap[dateStr] || []).length;
                const cell = document.createElement('div');
                cell.className = 'year-heat-cell' + (dateStr === todayStr ? ' today' : '');
                cell.dataset.date = dateStr;
                if (count > 0) {
                    const intensity = Math.min(1, 0.15 + (count / maxCount) * 0.85);
...
function renderCurrentView(container, dv, app, globalState) {
    ...
    if (!tasks.length) {
        viewArea.innerHTML = '<div class="empty-message">📭 无符合条件的任务</div>';
    ...
    // 从容器读取 groupCount 和 taskCount，更新标题
    const groupCount = parseInt(viewArea.dataset.groupCount || '0');
    const taskCount = parseInt(viewArea.dataset.taskCount || '0');
    const countSpan = container.querySelector('.cal-task-count');
    if (countSpan) {
        const title = buildGlobalTitle(safeGS, groupCount, taskCount);
        countSpan.textContent = '';
        // 标题已在外面，这里只更新统计部分？实际上 buildGlobalTitle 返回完整文本，需要重新设置标题元素的文本。
        // 我们希望标题在 cal-global-title 内，分为两部分：前缀和统计。因此我们可以让 titleDiv 包含两个 span，一个放前缀，一个放统计。
        // 更简单：在 buildGlobalTitle 只生成统计字符串，然后在 renderCurrentView 中设置 titleSpan 和 countSpan。
        // 调整设计：在 createUI 时标题内容分为 prefixSpan 和 countSpan。后续更新 countSpan 即可。
    }
    // 因为我们已经有了 countSpan，可以直接设置其文本，而前缀在初始化时已设置，无需改动。
    if (countSpan) {
...
export async function startCalendarView(dv, app, container, globalState) {
    ...
    prefixSpan.textContent = buildGlobalTitle(stateForTitle, 0, 0).replace(/ \(.*\)$/, ''); // 去除默认统计
    ...
    toTopBtn.onclick = () => { viewArea.scrollIntoView({ behavior: 'smooth', block: 'start' }); };
    const toBottomBtn = document.createElement('button');
    toBottomBtn.textContent = '⏬ 页尾';
    toBottomBtn.onclick = () => { viewArea.scrollIntoView({ behavior: 'smooth', block: 'end' }); };
    ...
    return {
        ...
        updateSort: () => { renderCurrentView(container, dv, app, globalState); }
    };
}