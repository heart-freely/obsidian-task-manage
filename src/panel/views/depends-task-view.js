...
export class DependsTaskView extends BaseTaskView {
    getViewType() { return VIEW_TYPE_DEPENDS; }
    getDisplayText() { return '依赖任务'; }
    getIcon() { return 'link'; }
...
export async function startDependsView(dv, app, container) {
    async function render() {
        ...
        try {
            ...
            const dependsTasks = allTasks.filter(t => t._forbid && t._forbid.trim());

            if (!dependsTasks.length) {
                container.innerHTML = '<div class="empty-placeholder">🔗 暂无依赖任务</div>';
                return;
            }

            const wrapper = document.createElement('div');
            wrapper.className = 'view-col';
            wrapper.style.setProperty('--quad-color', 'rgba(255, 200, 100, 0.25)');
