...
import { drawCharts } from "./views/data-base-tasks-view";
...
export class NavigatorView extends BaseTaskView {
	...
	getState() {
		return { subViewType: this._lastViewType || "data-base-tasks-view" };
	}
	setState(state) {
...
export async function startNavigatorCore(
	...
	async function refreshCurrentView() {
		...
		// 刷新甘特图/日历等需要同步的视图
		if (
			["calendar-task-view", "gantt-task-view"].includes(
				currentSubViewType,
			) &&
	...
	async function renderContent(filteredTasks) {
		...
		if (currentSubViewType === "data-base-tasks-view") {
	...
	if (
		...
		if (currentSubView && currentSubView.updateSort)
			await currentSubView.updateSort();
	} else if (currentSubViewType === "data-base-tasks-view") {
		await renderContent(filtered);
	}
