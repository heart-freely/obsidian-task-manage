// src/echarts/draw-echarts.js
// 图表渲染模块 —— 接收数据与容器，负责创建和更新图表
...
export function drawCharts(container, tasks, context) {
    ...
    ensureEcharts(echarts => {
        ...
        const statusCounts = {}; ALLOWED_STATUSES.forEach(s => statusCounts[s] = 0);
        const prioCounts = {}; PRIORITY_ORDER.forEach(p => prioCounts[p] = 0);
        const repeatCounts = {}; REPEAT_TYPES.forEach(r => repeatCounts[r] = 0);
        const dateCounts = {}; DATE_MARK_ORDER.forEach(m => dateCounts[m] = 0);
        ...
        tasks.forEach(t => {
            ...
            if (t._repeat) {
                REPEAT_TYPES.forEach(r => {
                    if (t._repeat.toLowerCase().includes(r)) repeatCounts[r]++;
                });
            }
            DATE_MARK_ORDER.forEach(m => { if (t['_' + m]) dateCounts[m]++; });
        ...
        i1.setOption(makePieOption(ALLOWED_STATUSES.map(s => ({
        ...
        i2.setOption(makePieOption(PRIORITY_ORDER.map((p, i) => ({
            name: p,
            value: prioCounts[p] || 0,
            itemStyle: { color: PRIORITY_COLORS[i] }
        })).filter(d => d.value > 0)));
        ...
        i3.setOption(makePieOption(REPEAT_TYPES.map((r, i) => ({
            name: '🔁 ' + r,
            value: repeatCounts[r],
            itemStyle: { color: REPEAT_COLORS[i] }
        })).filter(d => d.value > 0)));
        ...
        i4.setOption(makePieOption(DATE_MARK_ORDER.map((m, i) => ({
            name: DATE_MARK_NAMES[m],
            value: dateCounts[m],
            itemStyle: { color: DATE_MARK_COLORS[i] }
        })).filter(d => d.value > 0)));
        ...
        i10.setOption({
            ...
            legend: {
                data: stackData.statusOrder.map(s => STATUS_ICONS[s] + STATUS_NAMES[s]),
                bottom: 0,
                textStyle: { color: textColor }
            },
