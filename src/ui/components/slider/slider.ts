...
export function createSlider(options: SliderOptions): {
	...
	const createHandle = (pct: number, isStart: boolean): HTMLElement => {
		...
		el.style.cssText = `position:absolute;top:-6px;left:${pct}%;width:6px;height:16px;background:var(--interactive-accent);border-radius:${radius};cursor:grab;transform:${translate};z-index:2;`;
		return el;
	};
