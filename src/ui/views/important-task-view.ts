...
export class ImportantTaskView extends BaseTaskView {
    getViewType() { return VIEW_TYPE_IMPORTANT; }
    getDisplayText() { return '重要任务'; }
    getIcon() { return 'star'; }
...
export async function startImportantView(dv, app, container) {
    async function renderImportant() {
        container.innerHTML = '';
        let data;
        try { data = await fetchImportantTasksByStatus(app); } catch (e) {
    ...
    return { cleanup: () => { container.innerHTML = ''; }, updateSort: () => { renderImportant(); } };
}