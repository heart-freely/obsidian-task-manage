...
import { createTaskCard, normalizeTaskCardData, adaptTasksApiTask } from './base-task-view';

export async function startListBaseView(app, container, fetchFn, title, color = 'var(--background-modifier-border)') {
    container.innerHTML = '';
    try {
        ...
        rawTasks.forEach(task => {
            ...
            const cardData = normalizeTaskCardData({
                description: adapted.description || adapted._cleanText || '（无描述）',
                priority: adapted.priority || 'none',
                status: mapSymbolToStatus(adapted.status?.symbol),
                recurrenceLabel: adapted.recurrence ? `🔁 ${adapted.recurrence.toText()}` : (adapted._repeat ? `🔁 ${adapted._repeat}` : ''),
                scheduled: adapted.scheduledDate ? window.moment(adapted.scheduledDate).format('YYYY-MM-DD') : null,
                start: adapted.startDate ? window.moment(adapted.startDate).format('YYYY-MM-DD') : null,
                due: adapted.dueDate ? window.moment(adapted.dueDate).format('YYYY-MM-DD') : null,
