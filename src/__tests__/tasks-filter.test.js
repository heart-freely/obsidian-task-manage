...
describe('filterTasks', () => {
    ...
    test('filters by root path', () => {
        const tasks = [
            makeTask({ path: 'pages/A 系统/task.md' }),
            makeTask({ path: 'pages/B 系统/task.md' }),
        ];
    ...
    test('filters by status', () => {
        const tasks = [
            makeTask({ _status: 'todo' }),
            makeTask({ _status: 'completed' }),
        ];
        const result = filterTasks(tasks, {
            markFilterState: { statuses: ['todo'] },
        });
        expect(result).toHaveLength(1);
    });

    test('hides repeat tasks when set', () => {
        const tasks = [
            makeTask({ _repeat: 'every day' }),
            makeTask({ _repeat: null }),
        ];
    ...
    test('include marks filter works (all must match)', () => {
        const tasks = [
            makeTask({
                _marks: { priority: true, tag: true },
            }),
            makeTask({
                _marks: { priority: false, tag: true },
            }),
        ];
        const result = filterTasks(tasks, {
            markFilterState: {
                includeMarks: ['priority', 'tag'],
                statuses: ['todo', 'planned', 'in-progress', 'completed', 'cancelled'], // all
            },
        });
        expect(result).toHaveLength(1);
    });
