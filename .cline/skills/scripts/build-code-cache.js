...
const COMMENT_BLACKLIST = [
	...
	"提取所有 @auto-* 和 @auto-anchor 注释及其行号",
	"写入 .cline/skills/cache/code_cache.json",
	"从 JS 文件内容中提取 @auto-* 和 @auto-anchor 注释",
];
...
/**
 * 从 JS 源文件中提取所有 @auto-* / @auto-anchor 注释。
 */
	...
	for (let i = 0; i < lines.length; i++) {
		...
		if (inBlockComment) {
			...
			if (line.includes("*/")) {
				...
				if (
					...
					/@auto-(?!anchor\b)|@auto-anchor/.test(fullText)
				) {
					commentBlocks.push({
		...
		if (singleIdx !== -1 && isLineComment(line, singleIdx)) {
			...
			if (
				...
				/@auto-(?!anchor\b)|@auto-anchor/.test(commentText)
			) {
