...
- **锚点初始化**：为源码文件添加 `// @skill-anchor:` 注释，用于精确定位。
...
- 扫描所有在 `.cline/skills/skills.md` 映射表中登记的源码文件。
- 为每个函数、类、状态对象生成 `// @skill-anchor: <标识>` 注释，插入到合适位置（函数前、类顶部等）。
- 更新 `.cline/skills/cache/code_cache.json`（原 `anchor_cache.json`）记录锚点位置。
...
3. 通过 `.cline/skills/skills.md` 映射表找到对应源码文件。
4. 读取源码全文。
...
1. **变更检测**：通过对话历史中的文件修改记录（`write_to_file`/`edit_file`）和文件修改时间对比（`.cline/skills/cache/sync_state.json` 中存储的 mtime），识别本次修改的 Skill 文件。仅处理位于 `docs/` 和 `.cline/skills/sync/` 下的 `.md` 文件。
...
4. **生成注释内容**：根据章节到标签的映射规则，将章节内容转换为 `@skill-*` 注释：
    ...
    - `## 状态模型 (@skill-state)` 下的代码块 → `/* @skill-state \n 原始内容\n */`
    - 其他类似。
5. **定位插入点**：
...
8. **更新缓存**：将插入/替换后的锚点行号写入 `code_cache.json`，并将新的注释内容解析结果写入 `parsed_cache.json`。
9. 输出同步摘要。
...
## 五、配置项（`.cline/skills/sync/sync_config.json`）
...
- 功能校验只读，不修改文件。
- 反向同步依赖 `.cline/skills/skills.md` 映射表。
