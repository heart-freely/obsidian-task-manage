...
- 扫描所有已映射的源码文件，映射关系查看`.cline/skills/skills.md`。
- 将 `// @skill-anchor: <标识>` 注释的行号和内容解析出来，注释规范参考`.cline/skills/sync/update-comment.md`。
- 将注释的行号和解析出来的内容更新到注释缓存文件 `.cline/skills/cache/code_cache.json`。
  - 如果`.cline/skills/cache/code_cache.json`不存在则创建，存在则更新。
...
4. **生成注释内容**：根据章节到标签的映射规则，将章节内容转换为 `@skill-*` 注释：
   ...
   - **生成头部注释**：提取 `文件`、`描述`、`依赖`、`对外导出`、`注意事项`。
      ...
      - **生成`@skill-*` 标签**：`@skill-sig`、`@skill-dom`、`@skill-state`、`@skill-flow`、`@skill-condition`、`@skill-api`、`@skill-algorithm` 、`@skill-anchor`等直接读取其注释内容。
   


5. 更新注释缓存文件：`.cline/skills/cache/code_cache.json` 

   1. 对比注释缓存文件，如果上一步“生成注释内容”的 `@skill-*` 注释和 `.cline/skills/cache/code_cache.json`中内容含义不一样则替换，如果一样仅定位插入点。
6. **定位插入点**：
   - **优先**：读取 `.cline/skills/cache/code_cache.json` 中该源码文件的锚点位置（如 `"mapToQuadrant": 45`）。
...
- 完成后若 `autoUpdateIndex` 为 `true`，自动调用 `update-index.md` 和 `update-skills.md` 刷新索引和 README。
	...
	"comments": [
		{
			...
			"commentGroups": [
				{
					"position": "@skill-*注释的行号",
					"comment": "/* @skill-*注释 */"
				},
				{
					"position": "@skill-anchor: <标识>注释的行号",
