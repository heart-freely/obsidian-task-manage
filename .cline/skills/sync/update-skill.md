---
name: 更新技能
description: 正向同步（源码→Skill）与功能校验。支持单独校验；同步前可选提醒。全量同步后自动更新技能索引。
triggers:
  - 更新技能|同步技能|全局同步技能
  - 检查功能实现|功能校验
---
...
### 同步流程
- **变更检测**：`git status` + `.cline/skills/cache/sync_state.json`（mtime/hash）。
- **定位 Skill**：通过 `README.md` 映射。
- **信息提取**：
  - 头部（描述、依赖、导出、`@see`）
