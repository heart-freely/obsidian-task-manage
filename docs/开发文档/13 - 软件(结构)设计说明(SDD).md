...
| `src/ui/bars/config-bar.ts`          | 视图配置栏：名称/图标、导入导出、恢复默认、删除            |
...
- 排除 YAML 无任务格式元数据的文件（任务名称, 计划时间, 截止时间, 开始时间, 完成时间, 执行状态, 优先级, 父任务）
-  优先使用 YAML 的 父任务 字段（子文件声明父文件）。
...
支持在其他通用视图左侧渲染，在窗口中，窗口可展开或折叠，窗口右侧有拖动条（参考视图配置栏）
