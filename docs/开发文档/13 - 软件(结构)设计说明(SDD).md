...
| 任务视图 | 分组展示 17 种视图样式按钮，多选一，当前激活视图高亮，纯文字标签 |
...
| 变量                          | 用途                  | 说明                                 |
| ----------------------------- | --------------------- | ------------------------------------ |
| `dynamicStart` / `dynamicEnd` | 动态滑动条            | 纯手动控制，不从 Store 恢复          |
| `staticStart` / `staticEnd`   | 静态滑动条 + 筛选条件 | 从 Store 恢复，受动态时间单向同步    |
...
- **计划~截止 / 开始~完成**：切换时间计算模式。`scheduled-due`：起始=`_scheduled`，结束=`_due`（无`_due`回退到`_done`）。`starts-done`：起始=`_starts`，结束=`_done`（无`_done`回退到`_due`）。按钮始终高亮。

- **使用动态**：未选中时动态和静态滑动条独立操作，互不影响。选中时动态滑动条影响静态滑动条，静态滑动条不影响动态滑动条。状态持久化到预设的 `useDynamic` 字段。按钮位于动态单位切换按钮右侧，选中高亮。
...
- 默认状态：两个手柄在"本日"位置重合，起始手柄右边缘与结束手柄左边缘贴合。

**静态滑动条**：

- 范围：年固定为前5年到后5年。季/月/周/日同年为全范围（1~4、1~12、1~52/53、1~365/366）。跨年时范围累加所有涉及年份的单位总数。
- 跨年值计算：使用 `for` 循环累加各年份的实际值（季+4、月+12、周+weeksInYear、日+daysInYear）。
- 格式化：使用 `while` 循环逐年减去单位数来确定所属年份，确保不出现0值。
- 级联更新：拖动任一静态滑动条后，所有级别（年/季/月/周/日）同步刷新。
- 标记线：今天所在位置用强调色标记。
- 说明文字：单值格式如 `2026年`、`2026/2季`、`2026/6月`、`2026/23周`、`2026/6/1日`。范围格式如 `2026~2027年`。文字宽度 160px，不换行。

**滑动条通用设计**：

- 手柄：对称设计。起始手柄向左突出（`border-radius:3px 0 0 3px`，`translateX(-100%)`），结束手柄向右突出（`border-radius:0 3px 3px 0`，`translateX(0)`）。宽度 6px，高度 16px。最小时间段时两个手柄贴合在同一刻度。
- 轨道：高度 4px，填充条颜色 `var(--interactive-accent)`。
- 刻度标记：最多 20 个。普通刻度 1px×6px 灰色半透明。今天刻度 2px×12px 强调色。如果 step 跳过了 todayVal，单独添加今天刻度。
- 交互：拖动单个手柄只移动该手柄。点击两个手柄之间的线段整体拖动。点击范围外轨道就近移动一个手柄。
- 钳制：所有手柄位置统一钳制到 `[min, max]` 范围内，防止飞出轨道。
- 精度：`calcDynamicOffset` 使用 `Math.floor` 确保同一天偏移为 0。`getLevelValues` 中 `dayOfYear` 先对齐 `setStart`。

##### 任务视图栏

- **功能**：切换通用视图样式，多选一。共 17 种视图，分 4 组展示。
...
- **分组**：
  ...
  | -------- | ------------------------------------------------ |
  | 基础视图 | 列表、卡片、表格                                 |
  | 标记视图 | 状态、优先级、循环、时间轴、标签、唯一ID、引用ID |
  | 管理视图 | 看板、矩阵、任务树、甘特图、日历图               |
  | 统计视图 | 基础统计、详细统计                               |
...
- **视图列表**：
  ...
  | 卡片     | 基础 | 卡片网格布局                                         |
  | 表格     | 基础 | 可排序列表格                                         |
  | 状态     | 标记 | 按执行状态分组（未开始/计划中/进行中/已完成/已取消） |
  | 优先级   | 标记 | 按优先级分组（🔺⏫🔼🔽⏬无），高到低排列，组内高到低      |
  | 循环     | 标记 | 按循环周期分组                                       |
  | 时间轴   | 标记 | 按截止日期分组                                       |
  | 标签     | 标记 | 按自定义标签分组                                     |
  | 唯一ID   | 标记 | 带唯一ID的任务                                       |
  | 引用ID   | 标记 | 带依赖ID的任务                                       |
  | 看板     | 管理 | 三列看板（未开始/计划中/进行中）                     |
  | 矩阵     | 管理 | 四象限矩阵                                           |
  | 任务树   | 管理 | 按文件层级展示                                       |
  | 甘特图   | 管理 | Canvas 甘特图                                        |
  | 日历图   | 管理 | 日/周/月/季/年视图                                   |
...
- 时间计算模式切换后，筛选结果相应变化。
- 切换动态单位时只重建动态滑动条，静态滑动条保持不动。
- 时间栏面板切换/Store更新时，TimeBar 实例不被重复创建。
- 跨年时静态滑动条范围和值正确，格式化不出现0值。
- 所有滑动条今天刻度标记样式一致。
...
| 文件                                           | 职责                                                         |
| ---------------------------------------------- | ------------------------------------------------------------ |
| `src/ui/bars/bars.ts`                          | ToolbarManager：容器管理、样式注入、面板复用、Bar 实例缓存、拖拽高度、折叠、输入保护 |
| `src/ui/bars/head-bar.ts`                      | 标题栏按钮条：渲染、眼睛图标、拖拽排序、标签映射             |
| `src/ui/bars/time-bar.ts`                      | 任务时间栏：动态/静态滑动条、自定义滑动条组件、模式切换、刻度标记、跨年格式化 |
...
| `src/ui/bars/view-bar.ts`                      | 任务视图栏：4 组 17 种视图、纯文字按钮                       |
...
| `src/tasks/process/filter-task-process.ts`     | 筛选逻辑、日期计算、跨年偏移、范围计算                       |
| `src/tasks/process/common-process.ts`          | DateUtils 日期工具集                                         |
| `src/configs/configs.ts`                       | 全局常量、默认值                                             |
| `src/types.ts`                                 | 类型定义                                                     |
| `src/ui/components/lists/status-renderer.ts`   | 状态视图：按执行状态分组渲染                                 |
| `src/ui/components/lists/priority-renderer.ts` | 优先级视图：按优先级分组渲染，高到低排列                     |
| `src/ui/views/all-tasks-view.ts`               | 所有任务视图：17 种视图样式的调度                            |
