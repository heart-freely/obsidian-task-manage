...
侧边栏固定在插件布局的最左侧，与右侧的视图配置栏和通用视图区域协同工作。侧边栏的每个按钮代表一个**视图方案（Preset）**，点击按钮切换激活方案，右侧视图内容随之刷新。侧边栏顶部提供折叠/展开按钮和全局设置按钮，底部提供新建视图按钮。
...
视图设置栏是插件工具栏的核心配置区域，位于每个业务视图顶部，由标题栏按钮条和可展开的功能面板组成。其职责是提供筛选、排序、视图切换等配置能力，自身不展示任务内容，而是将用户的配置作用于下方的通用视图区域。
...
| 任务视图 | 分组展示 15 种视图样式按钮，多选一，当前激活视图高亮，纯文字标签 |
| 视图隐藏 | 控制循环/已完成/已取消/文件夹及表格列的显示或隐藏。多选，隐藏状态下高亮（文字"显示xx"），默认全部隐藏 |
| 视图排序 | 提供 14 种排序选项，多选一，支持正序/逆序切换，当前排序按钮追加"↑"或"↓" |
| 视图配置 | 导入/导出配置、恢复出厂默认（含标记栏全选状态）、保存配置、删除视图、设置视图名称和图标 |

#### 2.3 配置持久化概要

每个预设独立保存完整的工具栏状态（`showToolbar`、`toolbarOrder`、`barVisibility`、面板高度/折叠、`tableColumns`、筛选/排序/视图样式等）。切换预设时若新预设从未打开过工具栏而当前已打开，则自动开启（智能继承）。插件卸载时草稿自动合并保存，重启通过 `wasViewOpen` 恢复视图，配置合并默认值保证升级兼容。导入/导出为 JSON 文件。
...
| `refreshContent(): void`                                     | 刷新面板内容，复用已缓存面板或创建新面板并注入对应 Bar 组件。检测输入框聚焦状态，若聚焦则跳过面板内容重建 |
| `updatePreset(changes: Partial<any>): void`                  | 更新当前预设的部分字段到 Store                               |
| `togglePanels(): void`                                       | 切换面板的折叠/展开状态                                      |
| `showPanels(): void`                                         | 若面板当前折叠，则展开面板                                   |
| `cleanupAll(): void`                                         | 清理所有 DOM 元素、事件监听、样式标签、面板缓存              |
...
其中 `container` 为面板内用于渲染内容的容器元素，`store` 为全局状态管理器。组件内部通过 `store.subscribe()` 监听变化并自动重渲染。
...
| `toolbarEverShown`       | `boolean`                                  | 是否至少展开过一次工具栏                         |
| `viewIcons`              | `Record<string, string>`                   | 视图按钮自定义图标映射                           |
...
处理：
  ...
  3. 遍历 barPanels（Map<string, HTMLElement>）：
     a. 若 key 不在 visibleKeys 中，移除面板 DOM，从 Map 中删除
  4. 遍历 visibleKeys：
     a. 若 Map 中已有对应面板，清空面板内容
     b. 若 Map 中无对应面板，创建新 div，加入 panelsContainer，存入 Map
     c. 实例化对应的 Bar 组件（new BAR_COMPONENTS[key](panel, store)）
输出：面板容器内容更新，无闪烁
```

> 示例：用户从"所有任务"视图切换到"待办任务"视图时，两个视图都显示"任务状态"面板，则面板 DOM 被复用，仅清空内容后重新渲染 ExcutBar 组件，而非销毁再创建。
...
处理：
  1. 若存在 draftFilter，将其合并到 oldPreset 的 filter 中
  2. 查找 oldPreset 和 targetPreset
  3. 若 oldPreset.showToolbar === true
...
本章是模块的 UI 规格说明，涵盖整体架构关系、标题栏与面板的样式规范、8 个子功能栏的具体布局，以及核心交互流程和样式管理机制。本章内容层次如下：

- **6.1 整体架构** — 与侧边栏、视图内容的关系及对齐策略
- **6.2 标题栏设计** — 按钮组成、标签映射、交互行为、样式
- **6.3 功能面板设计** — 容器规范、折叠展开机制、面板复用
- **6.4 通用 UI 规范** — 按钮、说明文字、行容器的统一标准
- **6.5 子功能栏详细设计** — 8 个功能栏的具体按钮、标签、交互
- **6.6 核心交互流程** — 打开/关闭、折叠/展开、拖拽等操作流程
- **6.7 样式管理** — 样式的注入和清理机制
...
- 切换预设或显隐时，仅移除不需要的面板，保留的容器清空内容后重新渲染，避免闪烁。
...
- **功能**：提供动态时间滑动条和静态时间滑动条，支持年/季/月/周/日多层级时间范围选择。所有滑动条共享同一对日期（`startDate` 和 `endDate`）。
...
- **模式切换**：
  ...
  - **计划~截止 / 开始~完成**：切换时间计算模式。`scheduled-due`：起始=`_scheduled`，结束=`_due`（无`_due`回退到`_done`）。`starts-done`：起始=`_starts`，结束=`_done`（无`_done`回退到`_due`）。起止字段都无时不参与日期筛选。

  - **使用动态**：未选中时动态和静态滑动条独立操作，互不影响。选中时动态滑动条影响静态滑动条，静态滑动条不影响动态滑动条。状态持久化到预设的 `useDynamic` 字段。

- **动态滑动条**：

  - 单位切换按钮：年/季/月/周/日，选中高亮，切换后滑动条范围和说明文字自动调整。
  - 范围：日/周/月/季不允许跨年，范围根据今天到年初/年末的距离动态计算。年允许跨年，范围固定为前后5年。
  - 中点标记：偏移为0的位置（今天）用强调色重点标记。
  - 说明文字：偏移为0时显示"本日/本周/本月/本季/本年"。非0时：日显示具体日期（xx/xx格式），周/月/季/年显示"前/后N天/周/月/季/年"。

- **静态滑动条**：

  - 范围：年固定为前5年到后5年。季/月/周/日固定为全范围（1~4、1~12、1~52/53、1~365/366）。跨年时最大值动态扩展以容纳偏移后的值。
  - 跨年偏移：跨年时给小年值加偏移（季+4、月+12、周+53、日+366）。显示时还原为实际值（如53周显示为1周）。还原时使用 `Math.max(1, v - offset)` 保护边界。
  - 标记线：今天所在位置用强调色标记。

- **滑动条通用设计**：

  - 双端拖拽：起始端不超过结束端，允许重合。拖动值严格限制在 `[min, max]` 范围内。
  - 刻度标记：轨道上标记整数值位置，最多20个刻度。普通刻度：1px宽×6px高，灰色半透明。今天刻度：2px宽×12px高，强调色。
  - 自动应用：拖动静态滑动条后自动更新筛选条件。
  - 精确计算：`calcDynamicOffset` 和 `maxDynamicRange` 使用一致的精确计算逻辑（`Math.floor` 而非 `Math.round`）。
...
任意滑动条拖动
  → setDates(lv, sv, ev)
  → datesFromLevel(lv, sv, ev)
  → 更新 this.startDate / this.endDate
  → updateAllUI() / updateStaticUIOnly() / updateDynamicUIOnly()
  → 同步刷新其他滑动条位置
```
...
- **分组**：
  ...
  | -------- | ---------------------------------------------- |
  | 基础视图 | 列表、卡片、表格                               |
  | 组织视图 | 看板、矩阵、循环、时间轴、标签、唯一ID、引用ID |
  | 高级视图 | 任务树、甘特图、日历图                         |
  | 统计视图 | 基础统计、详细统计                             |
...
- **隐藏控制行**：标签"隐藏"，四个按钮（动态文字）：
  ...
  各按钮独立，支持多选。默认全部选中（隐藏）。

- **表格列控制行**：标签"表格列"，各列按钮（状态、描述、优先级、循环、创建、计划、开始、截止、完成、取消、标签、唯一ID、引用ID），点击切换显隐。循环/创建/取消默认隐藏（高亮），标签/唯一ID/引用ID默认显示。
...
- **当前排序**：激活按钮追加"↑"或"↓"。点击未激活的按钮切换字段并默认升序，点击已激活的切换升降序。

- **标签**："排序"，标准样式。

##### 视图配置栏

- **功能**：管理视图的名称、图标，以及配置的导入、导出、恢复、保存和删除。

- **视图名称行**：标签"视图名称"，文本输入框，实时保存。

- **视图图标行**：标签"视图图标"，Emoji 输入框，用于设置侧边栏按钮图标。

- **操作按钮行**：标签"视图配置"，五个按钮从左至右：

  | 按钮       | 功能                                 |
  | ---------- | ------------------------------------ |
  | 📥 导入配置 | 从 JSON 文件导入并合并至当前预设     |
  | 📤 导出配置 | 导出当前预设为 JSON 文件             |
  | 🔄 恢复默认 | 重置所有状态为出厂默认，清除草稿     |
  | 💾 保存配置 | 将草稿筛选合并至预设并持久化         |
  | 🗑️ 删除视图 | 删除当前视图，自动切至剩余第一个视图 |

- **恢复默认**：将当前视图所有状态（筛选条件、排序、视图样式、工具栏顺序、显隐、标记栏选中状态、intervalMode、useDynamic 等）重置为出厂默认值，同时清除未保存草稿。
...
- **Store 依赖**：假定 Store 已在插件加载时完成初始化并传入。面板显隐和排序需要通过 Store 持久化，Store 必须支持 `update`、`subscribe`、`getState` 方法。
- **浏览器兼容性**：假定运行环境为 Chromium（Electron），支持 ES2020+、ResizeObserver、CSS Flexbox、`text-align-last` 等特性。
- **性能假设**：面板数量固定（8个），单个面板内按钮数量不超过 50 个，面板复用机制已保证切换时无明显性能问题。
- **并发假设**：Store 的订阅通知为同步调用，面板刷新在同一事件循环内完成。
- **移动端适配**：当前未充分测试移动端，工具栏在小屏幕设备上可能存在布局问题。

### 8. 测试要点

- 切换侧边栏视图按钮时，配置栏状态正确恢复（显隐、折叠、高度、顺序）。
- 拖拽标题栏按钮后，面板排列顺序同步更新。
- 点击眼睛按钮显隐面板，折叠状态下面板不渲染。
- 拖拽手柄调整面板高度，释放后持久化。
- 点击"恢复默认"后，所有状态（含标记栏选中）重置为出厂默认，草稿被清除。
- 切换预设时，工具栏智能继承逻辑正确触发。
- 插件卸载后，注入的样式标签和 DOM 元素被完全移除。
- 搜索框输入时，面板不闪烁，输入焦点不丢失。
- 标记栏全选时不过滤；取消某个标记后任务列表相应更新。
...
| 文件                                       | 职责                                                         |
| ------------------------------------------ | ------------------------------------------------------------ |
| `src/ui/bars/bars.ts`                      | ToolbarManager：容器管理、样式注入、面板复用、拖拽高度、折叠、输入保护 |
| `src/ui/bars/head-bar.ts`                  | 标题栏按钮条：渲染、眼睛图标、拖拽排序、标签映射             |
| `src/ui/bars/time-bar.ts`                  | 任务时间栏：动态/静态滑动条、模式切换、刻度标记              |
| `src/ui/bars/excut-bar.ts`                 | 任务状态栏：状态多选按钮                                     |
| `src/ui/bars/search-bar.ts`                | 任务描述栏：文本输入过滤、输入保护                           |
| `src/ui/bars/mark-bar.ts`                  | 任务标记栏：分组、主按钮全选/全不选、子按钮多选、优先级逆序  |
| `src/ui/bars/view-bar.ts`                  | 任务视图栏：分组、纯文字按钮                                 |
| `src/ui/bars/hide-bar.ts`                  | 视图隐藏栏：任务/文件夹显隐、表格列控制                      |
| `src/ui/bars/sort-bar.ts`                  | 视图排序栏：14 种排序、升降序                                |
| `src/ui/bars/config-bar.ts`                | 视图配置栏：名称/图标、导入导出、恢复默认、保存删除          |
| `src/ui/bars/side-bar.ts`                  | 侧边栏：视图列表、折叠展开、新建                             |
| `src/ui/ui.ts`                             | 主布局入口，创建宿主容器                                     |
| `src/store/store.ts`                       | Store：智能继承、草稿合并、持久化                            |
| `src/main.ts`                              | 插件入口：默认预设、wasViewOpen、全局设置                    |
| `src/settings.ts`                          | 全局设置面板                                                 |
| `src/tasks/process/filter-task-process.ts` | 筛选逻辑、日期计算、精确偏移计算                             |
| `src/configs/configs.ts`                   | 全局常量、默认值                                             |
| `src/types.ts`                             | 类型定义                                                     |
