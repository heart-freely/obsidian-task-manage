...
所有按钮，在所有查询到的任务中计算任务时间范围，任务标记时间最小的时间~任务标记时间最大的时间
...
| 功能栏   | 核心能力                                                     |
| -------- | ------------------------------------------------------------ |
| 任务状态 | 按执行状态（未开始/计划中/进行中/已完成/已取消）多选筛选，默认全选 |
| 任务描述 | 输入关键词实时过滤任务描述文本                               |
| 任务标记 | 按标记类型筛选，支持优先级和循环的具体值多选                 |
| 任务时间 | 动态日期（过去/现在/未来/全时）、静态级联日期（年/季/月/周/周几）、自定义范围、时间计算模式切换（计划~截止 / 开始~完成） |
| 任务视图 | 分组展示 15 种视图样式按钮，切换通用视图渲染方式             |
| 视图隐藏 | 控制循环/已完成/已取消/文件夹及表格列的显示或隐藏            |
| 视图排序 | 提供 14 种排序选项，支持正序/逆序切换，多选一                |
| 视图配置 | 导入/导出配置、恢复出厂默认、保存配置、删除视图、设置视图名称和图标 |
...
| `refreshContent(): void`                                     | 刷新面板内容，复用已缓存面板或创建新面板并注入对应 Bar 组件  |
...
处理：
  ...
  2. 遍历 barPanels（Map<string, HTMLElement>）：
...
- **功能**：按执行状态筛选，多选。默认全选。
- **按钮**：从左到右依次为“未开始”“计划中”“进行中”“已完成”“已取消”。选中高亮，点击切换。
- **标签**：“任务状态”，标准样式。

##### 任务描述栏

- **功能**：按关键词实时过滤任务描述文本。
- **组件**：一个文本输入框，占位文字“输入任务描述关键词…”，输入即更新筛选。
- **标签**：“任务描述”，标准样式。

##### 任务标记栏

- **功能**：按标记筛选，支持优先级和循环的具体值多选。默认所有标记参与筛选。
...
- **主按钮**：点击加入/移出 `includeMarks`。

- **子按钮**：始终可见，多选，高亮表示选中。优先级逆序排列，循环顺序排列。选中的值存入 `priorityValues` 或 `repeatCycles`。

- **标签**：标准样式。

##### 任务时间栏

- **功能**：提供动态日期范围、静态级联日期、自定义日期范围以及时间计算模式切换。
...
- **动态日期分组**（四行）：
  ...
  默认选中“本周”，与静态过滤二选一。点击按钮设置日期范围（如“今天”为当天00:00:00~23:59:59，“所有”为任务标记时间的最小~最大）。
...
- **静态级联日期**（五行）：
  ...
  默认不选中。多级联动：年份单选；季度可多选，若选一则可进一步选月份/周数/周几；月份若选一则可选周数/周几；周数选一则可选周几（单选或多选）；周几可多选。一旦多选则无法向下细化。

- **自定义范围**：开始/结束日期选择器 + 应用按钮。其后的模式切换按钮显示“计划~截止”或“开始~完成”（无 emoji），默认“计划~截止”。时间计算方式：使用计划时间~截止时间判断任务与日历时间有交集；切换后使用开始时间~完成时间判断（若缺少完成时间则用截止时间代替）。

- 内部容器 `filter-section` 已重置边距，保证对齐。
...
- **隐藏控制行**：标签“隐藏”，四个按钮（动态文字）：
  ...
  各按钮独立，支持多选（同时隐藏多种）。按钮文字随状态切换改变。

- **表格列控制行**：标签“表格列”，各列按钮（状态、描述、优先级、循环、创建、计划、开始、截止、完成、取消），点击切换显隐，隐藏时按钮文字显示“隐藏XXX”。

- **标签**：标准样式。

##### 视图排序栏

- **功能**：按指定字段排序，正逆序切换，多选一。排序规则默认按计划日期从近到远。
- **排序选项**（14 个）：状态、描述、优先级、循环、创建、计划、开始、截止、取消、完成、标签、唯一ID、引用ID、文件名。点击分组按钮可展开/收缩其下的子按钮（默认全部展开），只能选中一个排序字段。
...
- **视图图标行**：标签“视图图标”，Emoji 输入框，用于设置侧边栏按钮图标。同时支持自定义通用视图图标（为每个视图样式单独设置图标）。
...
| 文件                        | 职责                                                         |
| --------------------------- | ------------------------------------------------------------ |
| `src/ui/bars/bars.ts`       | ToolbarManager：容器管理、样式注入、面板复用、拖拽高度、折叠 |
| `src/ui/bars/head-bar.ts`   | 标题栏按钮条：渲染、眼睛图标、拖拽排序、标签映射             |
| `src/ui/bars/time-bar.ts`   | 任务时间栏：动态/静态日期、自定义范围、模式切换              |
| `src/ui/bars/excut-bar.ts`  | 任务状态栏：状态多选按钮                                     |
| `src/ui/bars/search-bar.ts` | 任务描述栏：文本输入过滤                                     |
| `src/ui/bars/mark-bar.ts`   | 任务标记栏：分组、子按钮、优先级逆序                         |
